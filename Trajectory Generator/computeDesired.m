function desired = computeDesired(param,currentTime,set_traj,terminal_time,sampling_rate_dt,horiz, mode)
% This function computes the value of the desired trajectory for the
% geometric control or the NMPC


param.terminalTime = terminal_time;
param.dt = sampling_rate_dt ;
param.horizon = horiz;
...
horizon = [0:param.dt:param.terminalTime]';   
end
...
ud = [];
fM = [];
for k = 1:(length(horizon)-1)
    %       [p,v,R,omega,f,M] = differentialFlatnessFull(p,v,a,j,s,psi,psi_dot,psi_dotdot,param)
    [~,~,R,omega,f,M] = differentialFlatnessFull([xd(horizon(k)),yd(horizon(k)),zd(horizon(k))]',...
    ...
    fM = [fM; [f;M]'];
    %desired.ud = [desired.ud; singleRotorInput'];
    %desired.fM = [desired.fM ; [f;M]'];
end
 desired.ud = ud;
 desired.fM = fM;
 sz = size(horizon);
